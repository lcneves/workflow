...
Use `providerOptions` to pass provider-specific settings like OpenAI's reasoning effort or service tier:
...
async function agentWithProviderOptions(userMessage: string) {
  "use workflow";

  // Set default providerOptions in the constructor
  const agent = new DurableAgent({
  ...
  await agent.stream({
    messages: [{ role: "user", content: userMessage }],
    writable: getWritable<UIMessageChunk>(),
    // Override providerOptions per-call if needed
    providerOptions: {
      openai: {
