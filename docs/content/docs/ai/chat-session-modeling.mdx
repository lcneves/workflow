...
<Tabs items={['Workflow', 'API Routes', 'Hook Definition', 'useMultiTurnChat helper', 'Client']}>
...
<Tab value="useMultiTurnChat helper">
...
export function Chat({ initialMessages }) {
  const { messages, threadId, startSession, sendMessage, endSession, ...chatProps } = useMultiTurnChat();
}
```
