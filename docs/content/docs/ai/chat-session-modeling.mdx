...
We can replace our `useChat` call in `app/page.tsx` with a custom hook that manages the chat session. This hook will handle switching between the API endpoints for creating a new thread and sending follow-up messages.

```typescript title="components/use-multi-turn-chat.ts" lineNumbers
"use client";
