...
<Callout type="warn">
`@workflow/ai` is in public beta and should be considered experimental.
</Callout>
...
- **Durability**: Queue management, tracking sub-jobs, and handling task execution across failures
- **Reliability**: Error handling, retries, and recovery from partial failures
- **Observability**: Storing messages, emitting traces, and debugging agent behavior
- **Resumability**: Persisting state to a database, managing stream providers for reconnection
- **Long-running operations**: Sleep, delays, and month-long workflows without idle costs
- **Human approval loops**: Queues, async workers, and database-backed state management
