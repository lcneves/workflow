...
<Callout type="info">
O sistema de serialização garante que todos os dados persistam corretamente durante suspensões e retomadas de workflows, possibilitando execução durável.
</Callout>
...
<Callout type="info">
Estes tipos têm tratamento especial e são explicados em detalhe nas seções abaixo.
</Callout>
...
For complete information about using streams in workflows, including patterns for AI streaming, file processing, and progress updates, see the [Guia de Streaming](/docs/foundations/streaming).
...
## Semântica de passagem por valor
